...
export class AddMovieComponent implements OnInit {
  ...
  messages = {
    movieName: {
      requierd: 'Movie name is required',
      Exists: 'The movie name already exists, please choose another'
    },
    story: {
      requierd: 'Story is required'
    },
    trailer: {
      requierd: 'Wanted movie announcement',
      valid: 'The entered link is invalid'
    },
    catId: {
      requierd: 'Classification required'
    },
    post: {
      requierd: 'Wanted movie poster'
    },
    actorId: {
      requierd: 'Film actor name is required',
      Exists: 'An actor already added'
    },
    film: {
      requierd: 'Wanted movie'
    },
    link: {
      requierd: 'The download link is invalid'
    },
  }
  ...
  AddMovie() {
    ...
    if (this.movieForm.valid && this.actorIds.length > 0 && !this.isMovieExists(movName)) {
      ...
      this.adminService.AddMovie(fd).subscribe(success => {
        this.sucMsg = 'The movie was added successfully';
                this.errMsg = null;
      }, ex => {
        console.log(ex);
        this.sucMsg = null;
